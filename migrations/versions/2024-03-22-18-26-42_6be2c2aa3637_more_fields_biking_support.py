...
def upgrade():
    ...
    if engine_name == 'postgresql':
        sa.Enum('CAR', 'BIKE', 'ITEM', name='purposetype').create(op.get_bind())
        ...
        sa.Enum('ON_STREET', 'OFF_STREET_PARKING_GROUND', 'UNDERGROUND', 'CAR_PARK', 'OTHER', name='_parkingsitetype').drop(op.get_bind())
    ...
    with op.batch_alter_table('parking_site', schema=None) as batch_op:
        ...
        batch_op.alter_column(
            'type',
            existing_type=sa.Enum('ON_STREET', 'OFF_STREET_PARKING_GROUND', 'UNDERGROUND', 'CAR_PARK', 'OTHER', name='parkingsitetype'),
            type_=sa.Enum(
                'ON_STREET',
                'OFF_STREET_PARKING_GROUND',
                'UNDERGROUND',
...
def downgrade():
    ...
    op.execute('ALTER TYPE parkingsitetype RENAME TO _parkingsitetype')
    sa.Enum('ON_STREET', 'OFF_STREET_PARKING_GROUND', 'UNDERGROUND', 'CAR_PARK', 'OTHER', name='parkingsitetype').create(op.get_bind())
    op.execute('ALTER TABLE parking_site ALTER COLUMN type type parkingsitetype using type::text::parkingsitetype;')
    sa.Enum('ON_STREET', 'OFF_STREET_PARKING_GROUND', 'UNDERGROUND', 'CAR_PARK', 'OTHER', name='_parkingsitetype').drop(op.get_bind())
    ...
    with op.batch_alter_table('parking_site', schema=None) as batch_op:
        ...
        batch_op.alter_column(
            ...
            type_=sa.Enum('ON_STREET', 'OFF_STREET_PARKING_GROUND', 'UNDERGROUND', 'CAR_PARK', 'OTHER', name='parkingsitetype'),
            existing_nullable=True,
        )
    ...
    # If it's a postgre database, we need to delete enums manually
    engine_name = op.get_bind().engine.name
    if engine_name == 'postgresql':
        sa.Enum('OSM', 'DHID', name='externalidentifiertype').drop(op.get_bind())
        sa.Enum('CAR', 'BIKE', 'ITEM', name='purposetype').drop(op.get_bind())
        sa.Enum('YES', 'NO', 'VIDEO', 'ATTENDED', name='supervisiontype').drop(op.get_bind())
    # ### end Alembic commands ###
