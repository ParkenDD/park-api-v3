...
def test_get_v1_pool(public_api_test_client: FlaskClient, multi_source_parking_site_test_data: None) -> None:
    response = public_api_test_client.get(path='/api/public/v2/pools/source-1/')
    assert response.status_code == 200
    assert response.json == PARK_API_V2_POOL_RESPONSE


def test_get_v1_lots(public_api_test_client: FlaskClient, multi_source_parking_site_test_data: None) -> None:
    response = public_api_test_client.get(path='/api/public/v2/lots/')
