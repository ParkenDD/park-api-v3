...
@pytest.fixture(scope='function')
def fill_database(db_noreset, parking_site: ParkingSite) -> ParkingSite:
    db_noreset.session.add(parking_site)
    db_noreset.session.commit()

    yield

    empty_tables(db_noreset, models=[Source, ParkingSite])


def test_get_data(test_client: FlaskClient, fill_database):
    """
