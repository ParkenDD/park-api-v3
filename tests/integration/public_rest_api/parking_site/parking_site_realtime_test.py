...
def test_realtime_data_outdated_calculation_disabled(
    ...
    response = public_api_test_client.get(path='/api/public/v3/parking-sites/1?calculate_has_realtime_data=false')
...
def test_realtime_data_outdated_calculation_disabled_in_list(
    ...
    response = public_api_test_client.get(path='/api/public/v3/parking-sites?calculate_has_realtime_data=false')

    assert response.status_code == 200
    item = response.json['items'][0]
    assert item['has_realtime_data'] is True
    assert item['realtime_capacity'] == 100
    assert item['realtime_free_capacity'] == 42
...
def test_realtime_data_outdated_calculation_enabled_explicitly(
    ...
    response = public_api_test_client.get(path='/api/public/v3/parking-sites/1?calculate_has_realtime_data=true')
